...
function ResearchCard({ a }: { a: ResearchItem }) {
  ...
  return (
    <Link href={`/research/${a.slug}`} style={{ ...card, padding: 0, textDecoration: 'none', display: 'block', overflow: 'hidden' }}>
      {a.heroImage ? (
        <Image src={a.heroImage} alt={a.title} width={1200} height={800} loading="lazy" sizes="(max-width: 640px) 100vw, (max-width: 1024px) 50vw, 400px" style={{ width: '100%', height: 'auto', display: 'block', background: '#0f172a' }} />
      ) : (
...
function InvestingCard({ slug, title, description, thumb, week, published, accentColor }: {
  ...
  const inner = (
    <div style={{ ...card, padding: 0, overflow: 'hidden', opacity: published ? 1 : 0.45 }}>
      <Image src={thumb} alt={title} width={1200} height={800} loading="lazy" sizes="(max-width: 640px) 100vw, (max-width: 1024px) 50vw, 400px" style={{ width: '100%', height: 'auto', display: 'block', background: '#0f172a' }} />
