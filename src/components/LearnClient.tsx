...
function ResearchCard({ a }: { a: ResearchItem }) {
  ...
  return (
    <Link href={`/research/${a.slug}`} style={{ ...card, padding: 0, textDecoration: 'none', display: 'block', overflow: 'hidden' }}>
      {a.heroImage ? (
        <Image src={a.heroImage} alt={a.title} width={1200} height={675} loading="lazy" sizes="(max-width: 640px) 100vw, (max-width: 1024px) 50vw, 400px" style={{ width: '100%', height: 'auto', display: 'block', background: '#0f172a' }} />
      ) : (
...
function LessonCard({ l }: { l: LessonItem }) {
  ...
  const inner = (
    <div style={{ ...card, padding: 0, overflow: 'hidden', opacity: published ? 1 : 0.45 }}>
      {l.thumb ? (
        <Image src={l.thumb} alt={l.thumbAlt || l.title} width={1200} height={675} loading="lazy" sizes="(max-width: 640px) 100vw, (max-width: 1024px) 50vw, 400px" style={{ width: '100%', height: 'auto', display: 'block', background: '#0f172a' }} />
      ) : (
...
function InvestingCard({ slug, title, description, thumb, week, published, accentColor }: {
  ...
  const inner = (
    <div style={{ ...card, padding: 0, overflow: 'hidden', opacity: published ? 1 : 0.45 }}>
      <Image src={thumb} alt={title} width={1200} height={675} loading="lazy" sizes="(max-width: 640px) 100vw, (max-width: 1024px) 50vw, 400px" style={{ width: '100%', height: 'auto', display: 'block', background: '#0f172a' }} />
      <div style={{ padding: '14px 16px 18px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 7, marginBottom: 8, flexWrap: 'wrap' }}>
...
const BEGINNER_LESSONS = [
  { week: 1, slug: 'investing-101-beginner-w1-what-is-a-stock-really', title: 'What Is a Stock, Really?', description: 'Forget the ticker symbols. A stock is a legal claim on a living business. Understand that, and everything else follows.', thumb: '/images/content/investing-101-beginner-w1-what-is-a-stock-hero.webp' },
  ...
  { week: 8, slug: 'investing-101-beginner-w8-valuation-basics', title: 'Valuation Basics: P/E, P/B, PEG', description: 'What does it mean for a stock to be "expensive" or "cheap"? Three core multiples and how to use them.', thumb: '/images/content/investing-101-beginner-w8-valuation-basics-hero.webp' },
  ...
  { week: 12, slug: 'investing-101-beginner-w12-your-first-five-years', title: 'Your First 5 Years: A Realistic Roadmap', description: 'A concrete action plan from Day 1 through Year 5. What to do, in what order, with what goals.', thumb: '/images/content/investing-101-beginner-w12-five-year-roadmap-hero.webp' },
];
