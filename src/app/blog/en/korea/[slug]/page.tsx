...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  return {
    ...
    openGraph: { title: `${post.title} | DHLM Studio`, description: post.description, locale: 'en_US' },
    alternates: { canonical: `https://dhlm-studio.com/blog/en/korea/${slug}` },
  };
}
...
export default async function KoreaPostPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <div style={{ background: '#FFFFFF' }}>
      <article className="mx-auto px-6" style={{ maxWidth: '720px', paddingTop: '80px', paddingBottom: '80px' }}>
        {/* Meta */}
        ...
        {related.length > 0 && (
          <div className="mt-14 pt-10" style={{ borderTop: '1px solid #E5E7EB' }}>
            ...
            <div className="space-y-4">
              {related.map((r) => (
                <Link key={r.slug} href={`/blog/en/korea/${r.slug}`}
                  className="block transition-colors duration-200 hover:text-[#C73E3A]"
                  style={{ color: '#1A1A1A' }}>
        ...
        <div className="mt-12 pt-8" style={{ borderTop: '1px solid #E5E7EB' }}>
          <Link href="/blog/en/korea" className="text-[13px] transition-colors hover:text-[#C73E3A]" style={{ color: '#BCBCBC' }}>
            ← All Korea articles
          </Link>
        </div>
      </article>
