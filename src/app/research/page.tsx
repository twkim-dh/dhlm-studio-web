...
function seriesColor(badge?: string) {
  ...
  return { fg: '#5B6470', bg: '#1E293B18', border: '#1E293B40' };
}
  ...
  return (
    <Link href={`/research/${a.slug}`} style={{ ...card, padding: 0, textDecoration: 'none', display: 'block', overflow: 'hidden', borderColor: c.border }}>
      {a.thumb ? (
        ...
        <img src={a.thumb} alt={a.title} loading="lazy" decoding="async" style={{ width: '100%', height: 'auto', display: 'block', background: '#FAFAF8' }} />
      ) : (
      ...
      <div style={{ padding: '16px 20px 20px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8, flexWrap: 'wrap' }}>
          ...
          <span style={{ fontSize: 10, color: '#8A929C', marginLeft: 'auto' }}>
            {fmtDateShort(a.date)} · {a.readTime}
          </span>
        </div>
        <div style={{ fontFamily: 'var(--serif)', fontSize: 18, fontWeight: 800, color: '#16161A', lineHeight: 1.4, marginBottom: 6 }}>{a.title}</div>
        <p style={{ fontSize: 12, color: '#8A929C', margin: 0, lineHeight: 1.55 }}>{a.description}</p>
      </div>
    </Link>
  );
}
    ...
    <div style={{ background: '#FFFFFF', minHeight: '100vh' }}>
      <div style={{ maxWidth: 800, margin: '0 auto', padding: '80px 24px' }}>
        ...
        <div style={{ marginBottom: 40 }}>
          ...
          <h1 style={{ fontFamily: 'var(--serif)', fontSize: 'clamp(26px, 4vw, 36px)', fontWeight: 400, color: '#16161A', margin: '0 0 10px', lineHeight: 1.2 }}>
            The Mental Game
          </h1>
          <p style={{ fontSize: 14, color: '#8A929C', lineHeight: 1.7, margin: 0, maxWidth: 560 }}>
        ...
        {other.length > 0 && (
          <section>
            <div style={{ fontFamily: 'var(--mono)', fontSize: 10, fontWeight: 800, color: '#8A929C', letterSpacing: 2, marginBottom: 14 }}>RESEARCH</div>
            <div className="research-grid">
        ...
        {articles.length === 0 && (
          <div style={{ ...card, padding: '40px 20px', textAlign: 'center' }}>
            ...
            <p style={{ fontSize: 14, color: '#8A929C' }}>Coming soon.</p>
          </div>
        )}

        <p style={{ fontSize: 9, color: '#8A929C', textAlign: 'center', marginTop: 40 }}>
