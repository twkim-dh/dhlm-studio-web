...
import { recentDraws, getNumberFrequency, getHotNumbers, getColdNumbers } from '@/data/lotto/recent-draws';
...
export default function StatsPage() {
  ...
  // Hot/cold for display with counts
  const hot = useMemo(() => {
    ...
    return hotNumbers.map((num) => ({ num, count: recentFreq[num] }));
  }, [hotNumbers]);

  const cold = useMemo(() => {
    return coldNumbers.map((num) => ({ num, count: 0 }));
  }, [coldNumbers]);
  ...
  return (
    <div className="px-4 pt-6">
      ...
      <p className="text-xs text-gray-400 text-center mb-4">
        최근 {recentDraws.length}회차 기준 (실제 당첨 데이터)
      </p>
      ...
      {tab === 'pairs' && (
        <div>
          <p className="text-xs text-gray-400 mb-4">
            최근 {recentDraws.length}회차에서 나란히 나온 연속 번호 쌍
          </p>
