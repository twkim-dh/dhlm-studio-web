...
function Ball({ n, max, size = 40, isBonus = false }: { n: number; max: number; size?: number; isBonus?: boolean }) {
  return (
    <div className="inline-flex items-center gap-1">
      {isBonus && <span className="font-bold text-xs" style={{ color: '#6B7280' }}>+</span>}
  ...
  return (
    <div className="min-h-screen" style={{ background: '#FFFFFF' }}>
      ...
      <div className="py-8 text-center">
        ...
        <p className="text-xs mt-1" style={{ color: '#6B7280' }}>{lottery.name}</p>
      ...
      <div className="max-w-lg mx-auto px-4 pb-8">
        ...
        {/* Info */}
        <div className="bg-white border rounded p-4 mb-4" style={{ borderColor: '#E5E7EB', borderRadius: '4px' }}>
          <div className="flex items-center gap-3">
            <span className="text-3xl">{lottery.country}</span>
            <div>
              <p className="font-bold text-base" style={{ color: '#1A1A1A' }}>{lottery.name}</p>
        ...
        <div className="flex gap-2 mb-4">
          <div className="flex items-center gap-1.5 bg-white border rounded px-3" style={{ borderColor: '#E5E7EB', borderRadius: '4px' }}>
            <span className="text-xs" style={{ color: '#6B7280' }}>{lottery.lang.sets}</span>
            {[1, 3, 5].map((n) => (
        ...
        {/* Results */}
        {results.length > 0 && (
          <div className="bg-white border rounded p-4 space-y-3" style={{ borderColor: '#E5E7EB', borderRadius: '4px' }}>
            {results.map((r, i) => (
            ...
            <div className="flex gap-2 pt-3 border-t border-[#E0D8CC]">
              ...
              }} className="flex-1 py-2.5 rounded text-sm font-medium transition" style={{ background: '#E8E2D8', borderRadius: '4px', color: '#1A1A1A' }}>
                📋 {lottery.lang.copy}
              </button>
              <button onClick={async () => {
                ...
                await navigator.clipboard.writeText(text); setMsg('Copied!'); setTimeout(() => setMsg(''), 2000);
              }} className="flex-1 py-2.5 rounded text-sm font-medium text-white transition" style={{ background: '#C73E3A', borderRadius: '4px' }}>
                {lottery.lang.share}
              </button>
            </div>
        ...
        {/* Other lotteries */}
        <div className="mt-8">
          <p className="text-xs font-medium mb-2" style={{ color: '#6B7280' }}>{lottery.lang.otherLotteries}</p>
          <div className="flex flex-wrap gap-1.5">
            <Link href="/lotto" className="px-2.5 py-1 bg-white border rounded text-[10px] transition hover:border-[#C5981A]"
              style={{ color: '#1A1A1A', borderColor: '#E5E7EB', borderRadius: '4px' }}>
              🇰🇷 Korea 6/45
            </Link>
