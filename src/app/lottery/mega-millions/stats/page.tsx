...
export default function MegaMillionsStats() {
  ...
  const pbStats = analyzeSpecialBall(draws, 25, limit);
  const freq = getFullFrequency(draws, 70, limit);
  const combo = analyzeCombo(draws, 70, limit);
    ...
    <div style={{ background: '#0B0F19', minHeight: '100vh' }}>
      <div style={{ maxWidth: 800, margin: '0 auto', padding: '80px 24px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
          <div>
            <div style={{ fontFamily: 'var(--mono)', fontSize: 10, fontWeight: 700, color: '#F59E0B', letterSpacing: 3, marginBottom: 4 }}>🟡 MEGA MILLIONS STATISTICS</div>
        ...
        <div style={{ display: 'flex', gap: 4, marginBottom: 16, flexWrap: 'wrap' }}>
          {([['hot','🔥 Hot'],['cold','❄️ Cold'],['overdue','⏰ Overdue'],['ball','🟡 MB Ball'],['heatmap','🗺️ Heatmap'],['combo','📊 Patterns'],['history','📋 History']] as [typeof tab, string][]).map(([id,label]) => (
            <button key={id} onClick={() => setTab(id)} style={{ padding: '6px 12px', borderRadius: 8, border: 'none', cursor: 'pointer', background: tab === id ? '#F59E0B20' : '#111827', color: tab === id ? '#F59E0B' : '#64748B', fontSize: 11, fontWeight: tab === id ? 700 : 500 }}>{label}</button>
          ))}
        </div>
          ...
          <>
            ...
            {tab === 'hot' && (
              <div style={{ ...card, padding: '18px 20px' }}>
                <div style={{ fontFamily: 'var(--mono)', fontSize: 9, fontWeight: 700, color: '#F59E0B', letterSpacing: 2, marginBottom: 12 }}>🔥 HOT NUMBERS — Most Frequent (TOP 15)</div>
                <BarChart items={hot} color="#F59E0B" max={maxCount} />
            ...
            {tab === 'ball' && (
              <div style={{ ...card, padding: '18px 20px' }}>
                <div style={{ fontFamily: 'var(--mono)', fontSize: 9, fontWeight: 700, color: '#F59E0B', letterSpacing: 2, marginBottom: 12 }}>🟡 MEGA MILLIONS FREQUENCY (1-25)</div>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                  {pbStats.slice(0, 15).map(s => (
                    <div key={s.number} style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                      <div style={{ width: 26, height: 26, borderRadius: '50%', background: 'linear-gradient(135deg, #F59E0B, #D97706)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 800, fontSize: 10, color: '#fff', fontFamily: 'var(--mono)', flexShrink: 0 }}>{s.number}</div>
                      <div style={{ flex: 1, height: 12, borderRadius: 3, background: '#1E293B', overflow: 'hidden' }}>
                        <div style={{ height: '100%', borderRadius: 3, width: `${(s.count / (pbStats[0]?.count || 1)) * 100}%`, background: 'linear-gradient(90deg, #F59E0B, #FBBF24)' }} />
                      </div>
            ...
            {tab === 'combo' && (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                ...
                <div style={{ ...card, padding: '18px 20px' }}>
                  <div style={{ fontFamily: 'var(--mono)', fontSize: 9, fontWeight: 700, color: '#3B82F6', letterSpacing: 2, marginBottom: 10 }}>HIGH/LOW DISTRIBUTION (1-35 vs 36-70)</div>
                  ...
                  {draws.slice(0, Math.min(limit, 50)).map((d, i) => (
                    <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '8px 20px', borderBottom: '1px solid #1E293B15', fontSize: 11 }}>
                      ...
                      <div style={{ display: 'flex', gap: 3 }}>
                        ...
                        <span style={{ width: 24, height: 24, borderRadius: '50%', background: '#F59E0B', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', fontWeight: 700, fontSize: 10, color: '#fff', fontFamily: 'var(--mono)' }}>{d.special}</span>
                      </div>
        ...
        <div style={{ display: 'flex', gap: 6, marginTop: 16, flexWrap: 'wrap' }}>
          ...
          <Link href="/lottery/powerball/stats" style={{ fontSize: 11, color: '#F59E0B', padding: '5px 12px', borderRadius: 20, background: '#111827', border: '1px solid #1E293B' }}>🟡 PB Stats</Link>
        </div>
