...
export function generateStaticParams() {
  return Array.from({ length: 70 }, (_, i) => ({ num: String(i + 1) }));
}

function loadDraws(): DrawRecord[] {
  try {
    const filePath = path.join(process.cwd(), 'public/data/megamillions-history.json');
    const raw = JSON.parse(fs.readFileSync(filePath, 'utf8'));
    return raw.map((d: { date: string; numbers: number[]; mb: number }) => ({
      date: d.date, numbers: d.numbers, special: d.mb,
    }));
  } catch { return []; }
}
...
export default async function NumberDetailPage({ params }: { params: Promise<{ num: string }> }) {
  ...
  if (n < 1 || n > 70) return <div style={{ background: '#0B0F19', minHeight: '100vh', padding: '120px 24px', textAlign: 'center' }}><h1 style={{ color: '#F1F5F9' }}>Invalid Number</h1></div>;
  ...
  return (
    <div style={{ background: '#0B0F19', minHeight: '100vh' }}>
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify({
        ...
        name: `Mega Millions Number ${n} Statistics`,
        url: `https://dhlm-studio.com/lottery/mega-millions/number/${n}`,
      }) }} />
      <div style={{ maxWidth: 600, margin: '0 auto', padding: '80px 24px' }}>
        <Link href="/lottery/mega-millions/stats" style={{ fontSize: 12, color: '#64748B' }}>← Mega Millions Stats</Link>
        ...
        <div style={{ textAlign: 'center', margin: '24px 0' }}>
          ...
          <h1 style={{ fontFamily: 'var(--serif)', fontSize: 24, fontWeight: 900, color: '#F1F5F9', margin: '16px 0 4px' }}>Mega Millions Number {n}</h1>
          <p style={{ fontSize: 12, color: '#64748B' }}>Based on {draws.length} draws (2017–{YEAR})</p>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 8, marginBottom: 20 }}>
          <div style={{ ...card, padding: '14px 10px', textAlign: 'center' }}>
            ...
            <div style={{ fontSize: 28, fontWeight: 900, color: '#F59E0B', fontFamily: 'var(--mono)', marginTop: 4 }}>{stats.count}</div>
          ...
          <div style={{ ...card, padding: '18px 20px', marginBottom: 16 }}>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
              {stats.companions.map(c => (
                <Link key={c.number} href={`/lottery/mega-millions/number/${c.number}`} style={{
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 16 }}>
          {n > 1 && <Link href={`/lottery/mega-millions/number/${n - 1}`} style={{ fontSize: 12, color: '#64748B' }}>← #{n - 1}</Link>}
          <span />
          {n < 70 && <Link href={`/lottery/mega-millions/number/${n + 1}`} style={{ fontSize: 12, color: '#64748B' }}>#{n + 1} →</Link>}
        </div>
        ...
        <div style={{ ...card, padding: '14px 16px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(30px, 1fr))', gap: 3 }}>
            {Array.from({ length: 70 }, (_, i) => i + 1).map(num => (
              <Link key={num} href={`/lottery/mega-millions/number/${num}`} style={{
                ...
                background: num === n ? '#F59E0B' : '#1E293B', color: num === n ? '#fff' : '#94A3B8',
              }}>{num}</Link>
            ))}
          </div>
        </div>
