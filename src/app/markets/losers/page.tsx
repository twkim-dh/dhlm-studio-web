...
export default function LosersPage() {
  ...
  useEffect(() => {
    fetch('/api/markets')
      .then(r => r.json())
      .then(d => { if (d.losers?.length > 0) setItems(d.losers); })
      .catch(() => {})
    ...
    <div style={{ background: '#0B0F19', minHeight: '100vh' }}>
      <div style={{ maxWidth: 720, margin: '0 auto', padding: '80px 16px' }}>
        ...
        <div style={{ marginBottom: 20 }}>
          <div style={{ fontFamily: 'var(--mono)', fontSize: 10, fontWeight: 700, color: '#FF4545', letterSpacing: 3, marginBottom: 4 }}>🔴 TOP LOSERS · TODAY</div>
          <h1 style={{ fontFamily: 'var(--serif)', fontSize: 28, fontWeight: 900, color: '#F1F5F9', margin: 0 }}>Top Losers</h1>
          <p style={{ fontSize: 13, color: '#64748B', marginTop: 4 }}>Stocks with the biggest percentage drops today</p>
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
          {items.map((s, i) => (
            <Link key={s.ticker} href={`/markets/${s.ticker}`} style={{ ...card, display: 'flex', alignItems: 'center', gap: 12, padding: '14px 18px', textDecoration: 'none' }}>
              ...
              <div style={{ textAlign: 'right', flexShrink: 0 }}>
                ...
                <span style={{ fontSize: 12, fontWeight: 700, padding: '2px 7px', borderRadius: 5, background: '#FF45451A', color: '#FF4545', fontFamily: 'var(--mono)' }}>
                  {s.change.toFixed(1)}%
                </span>
              </div>
            </Link>
          ...
          {!loading && items.length === 0 && (
            <p style={{ fontSize: 13, color: '#475569', textAlign: 'center', padding: 40 }}>No losers data available. Try again later.</p>
          )}
        </div>
      </div>
    </div>
